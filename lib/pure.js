...
(function (root) {
  ...
  const LIMITS = {
    ...
    splashMs: 5000,         // the snake is held on screen this long: the scan itself takes about
                            // 30ms, too fast to read, and a click with no feedback feels broken.
                            // Set to one full turn of the scanning animation, whose keyframes are
                            // all 5s in panel.css. Shorten both together or the loop cuts off.
    splashReducedMs: 600,   // the hold when the reader asked for reduced motion. The animation is
                            // off for them, and holding a still picture for 5s is not respect,
                            // it is 5s of nothing happening.
    ...
    markSpotGrow: 46,       // px a finding's light pool reaches past it, in every lens
    heatMinSpot: 24,        // px floor on a Heat glow, so a thin finding still registers as a spot
    heatAreaGrow: 28,       // px a Heat area blob spreads past its element, so it smears into green
    heatGapGrow: 18,        // px a Heat gap core spreads, so the blur cannot wash the red out
